"""PIT-safe race-relative early-position history for PACE-01.

Each eligible past observation is the horse's first recorded passing position
expressed as a within-race percentile. One-segment records are structurally
ineligible, so Niigata turf straight 1000 m races do not masquerade as corner
positions. Every target row on a date is emitted before that date updates the
90-day state.
"""
...
PACE_RECENT_COLUMN = "pace__decay_90d__mean_early_position_percentile"
PACE_RECENT_TRANSFORMATION_HASH = "9b9dcadf4080a0dc059900848f5b2587d7f17ce980e37e86e12db7003a934ca5"
_PRESENCE_COLUMN = "_pace_recent_history_row_present"
_ELIGIBLE_PASSING_ORDER = re.compile(r"^[0-9]+(?:-[0-9]+)+$")


@dataclass(frozen=True)
class PaceRecentSpec:
    """Frozen PACE-01 transformation parameters."""
    ...
    def transformation_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "within_race_transform": {
                "checkpoint": "first recorded passing position",
            ...
            "output_column": PACE_RECENT_COLUMN,
        }


def _validate_frozen_transformation(spec: PaceRecentSpec) -> str:
    actual = canonical_json_hash(spec.transformation_dict())
    if actual != PACE_RECENT_TRANSFORMATION_HASH:
        raise ValueError(f"PACE-01 transformation differs from the preregistered hash: {actual}")
    return actual


def early_position_percentiles(starters: pd.DataFrame) -> pd.Series:
    """Return one race's frozen relative early-position observations."""

    required = {"started", "passing_order_raw"}
    missing = sorted(required.difference(starters.columns))
    if missing:
        raise ValueError(f"pace input is missing columns: {missing}")
    ...
    if n_valid < 2:
        return output
    first_position = pd.to_numeric(
        text.loc[valid].str.split("-").str[0],
        errors="raise",
    ).astype(float)
    if not np.isfinite(first_position.to_numpy()).all() or first_position.lt(1).any():
        raise ValueError("eligible passing positions must be finite positive integers")
    ranks = first_position.rank(method="average", ascending=True)
    output.loc[valid] = 1.0 - (ranks - 1.0) / (n_valid - 1.0)
    return output


def build_pace_recent_history(
    normalized: pd.DataFrame,
    *,
    spec: PaceRecentSpec | None = None,
    through_year: int = 2024,
) -> pd.DataFrame:
    """Emit PACE-01 history for every flat-race starter through 2024."""

    if through_year > 2024:
        raise ValueError("PACE-01 must not generate or inspect 2025+ history")
    spec = spec or PaceRecentSpec()
    ...
    if missing:
        raise ValueError(f"normalized PACE-01 input is missing columns: {missing}")
    ...
    for _, race in frame.groupby("race_id", sort=False):
        ...
        if starters.empty:
            continue
        starters["_pace_observation"] = early_position_percentiles(race).loc[starters.index]
    ...
    if ordered.duplicated(["race_id", "horse_id"]).any():
        raise ValueError("PACE-01 history contains duplicate runner keys")
    ...
    for event_date, day in ordered.groupby("race_date", sort=True):
        ...
        for index in day.index:
            observation = float(ordered.at[index, "_pace_observation"])
    ...
    ordered[PACE_RECENT_COLUMN] = np.asarray(emitted_values, dtype=np.float64)
...
def build_pace_recent_augmented_cache(
    baseline_cache_path: str | Path,
    pace_history: pd.DataFrame,
    output_path: str | Path,
    *,
    config: dict[str, Any],
) -> dict[str, Any]:
    """Append one PACE-01 column while preserving an arbitrary baseline cache."""
    ...
    if target.exists() or sidecar_path.exists():
        raise FileExistsError(f"refusing to overwrite PACE-01 cache: {target}")

    frame, metadata = read_model_frame_cache(baseline_path)
    if any(column.startswith("pace__") for column in frame.columns):
        raise ValueError("baseline cache already contains a pace column")
    required = {"race_id", "horse_id", PACE_RECENT_COLUMN, _PRESENCE_COLUMN}
    missing = sorted(required.difference(pace_history.columns))
    if missing:
        raise ValueError(f"PACE-01 history is missing columns: {missing}")
    if pace_history.duplicated(["race_id", "horse_id"]).any():
        raise ValueError("PACE-01 history contains duplicate runner keys")
    ...
    selected = pace_history.loc[
        :, ["race_id", "horse_id", PACE_RECENT_COLUMN, _PRESENCE_COLUMN]
    ].copy()
    ...
    if not augmented.loc[pre2025, _PRESENCE_COLUMN].eq(True).all():  # noqa: E712
        raise ValueError("PACE-01 history does not cover every pre-2025 model row")
    if augmented.loc[~pre2025, _PRESENCE_COLUMN].notna().any():
        raise ValueError("2025+ PACE-01 history rows must remain unavailable")
    augmented = augmented.drop(columns=_PRESENCE_COLUMN)
    augmented[PACE_RECENT_COLUMN] = pd.to_numeric(
        augmented[PACE_RECENT_COLUMN], errors="coerce"
    ).astype("float32")
    ...
    if not frame.loc[:, old_features].reset_index(drop=True).equals(augmented.loc[:, old_features]):
        raise ValueError("an existing baseline feature changed during PACE-01 augmentation")
    feature_columns = [*old_features, PACE_RECENT_COLUMN]
    groups = dict(metadata.get("feature_groups_v1", {}))
    groups["pace"] = [PACE_RECENT_COLUMN]
    ...
    return {
        ...
        "pre2025_nonmissing": int(augmented.loc[pre2025, PACE_RECENT_COLUMN].notna().sum()),
        "pre2025_missing": int(augmented.loc[pre2025, PACE_RECENT_COLUMN].isna().sum()),
        "retrospective_2025_feature_nonmissing": int(
            augmented.loc[~pre2025, PACE_RECENT_COLUMN].notna().sum()
        ),
    }


def load_pace_recent_config(path: str | Path) -> tuple[dict[str, Any], PaceRecentSpec]:
    """Load and validate the frozen PACE-01 preregistration."""

    config = json.loads(Path(path).read_text(encoding="utf-8"))
    if config.get("candidate_column") != PACE_RECENT_COLUMN:
        raise ValueError("registered PACE-01 column differs from code contract")
    raw = config.get("transformation")
    if not isinstance(raw, dict):
        raise ValueError("PACE-01 config is missing transformation")
    history = raw.get("history", {})
    spec = PaceRecentSpec(decay_half_life_days=int(history["half_life_days"]))
    if raw != spec.transformation_dict():
        raise ValueError("PACE-01 config transformation differs from code contract")
    actual = _validate_frozen_transformation(spec)
    if config.get("transformation_hash") != actual:
        raise ValueError("PACE-01 config transformation_hash is invalid")
    return config, spec


def build_pace_recent_cache_from_raw(
    *,
    repo_root: str | Path,
    raw_path: str | Path,
    baseline_cache_path: str | Path,
    ...
    """Build PACE-01 after removing 2025+ before normalization."""

    root = Path(repo_root).resolve()
    config_file = Path(config_path)
    if not config_file.is_absolute():
        config_file = root / config_file
    config, spec = load_pace_recent_config(config_file)
    ...
    history = build_pace_recent_history(normalize_raw(raw), spec=spec, through_year=2024)
    ...
    result["transformation_hash"] = PACE_RECENT_TRANSFORMATION_HASH
    return result
