...
from horse_pred.pace_final import build_pace_final_cache_from_raw
from horse_pred.pace_gain import build_pace_gain_cache_from_raw
...
def parser() -> argparse.ArgumentParser:
    ...
    s1 = commands.add_parser(
        "run-s1-two-axis-study",
        help="run the preregistered 2020-2022 two-axis race-value study",
    )
    s1.add_argument("--raw-path", type=Path, required=True)
    s1.add_argument("--output", type=Path, required=True)
    s1.add_argument("--repo-root", type=Path, default=Path.cwd())
    s1.add_argument(
        "--preregistration",
        type=Path,
        default=Path(
    ...
    pace_final = commands.add_parser(
        "build-pace-final-cache",
        help="build the preregistered one-column PACE-03 final-position cache",
    )
    pace_final.add_argument("--raw-path", type=Path, required=True)
    pace_final.add_argument("--baseline-cache", type=Path, required=True)
    pace_final.add_argument("--output", type=Path, required=True)
    pace_final.add_argument(
        "--config",
        type=Path,
        default=Path("configs/features/pace_03_final_position.json"),
    )
    pace_final.add_argument("--repo-root", type=Path, default=Path.cwd())
    pace_gain = commands.add_parser(
        "build-pace-gain-cache",
        help="build the preregistered one-column PACE-04 position-gain cache",
    )
    pace_gain.add_argument("--raw-path", type=Path, required=True)
    pace_gain.add_argument("--baseline-cache", type=Path, required=True)
    pace_gain.add_argument("--output", type=Path, required=True)
    pace_gain.add_argument(
        "--config",
        type=Path,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "build-pace-pressure-cache":
        ...
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return 0
    if args.command == "build-pace-final-cache":
