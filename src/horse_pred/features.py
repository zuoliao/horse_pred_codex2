...
def feature_groups(frame: pd.DataFrame) -> dict[str, tuple[str, ...]]:
    ...
    groups = {
        ...
        "rating_module": tuple(
            column for column in frame if column.startswith("modular_rating__")
        ),
        "margin_rating": tuple(
            column for column in frame if column.startswith("margin_rating__")
        ),
        ...
        "sectional": tuple(
            column for column in frame if column.startswith("sectional__")
        ),
        "pace": tuple(column for column in frame if column.startswith("pace__")),
        "pace_pressure": tuple(
            column for column in frame if column.startswith("pace_pressure__")
        ),
        "pace_final": tuple(
...
def semantic_feature_groups_v2(
    ...
    if any(column.startswith("modular_rating__") for column in feature_columns):
        groups["rating_module"] = []
    if any(column.startswith("margin_rating__") for column in feature_columns):
    ...
    if any(column.startswith("sectional__") for column in feature_columns):
        groups["sectional"] = []
    if any(column.startswith("pace__") for column in feature_columns):
        groups["pace"] = []
    if any(column.startswith("pace_pressure__") for column in feature_columns):
        groups["pace_pressure"] = []
    if any(column.startswith("pace_final__") for column in feature_columns):
        groups["pace_final"] = []
    ...
    for column in feature_columns:
        ...
        elif column.startswith("modular_rating__"):
            group = "rating_module"
        elif column.startswith("margin_rating__"):
            group = "margin_rating"
        ...
        elif column.startswith("sectional__"):
            group = "sectional"
        elif column.startswith("pace__"):
            group = "pace"
        elif column.startswith("pace_pressure__"):
            group = "pace_pressure"
        elif column.startswith("pace_final__"):
            group = "pace_final"
