...
The project is in **Phase 5A: systematic exploratory data analysis and problem reformulation**. Pause PV-06 continuation, margin-token refinement, rating transformations, COND-01, and every other local production-feature experiment until the EDA is complete. They are paused, not rejected. Do not automatically implement a production feature or full candidate model after EDA; present at most three prioritized hypotheses and wait for human review.

Use `docs/eda/02_eda_protocol.md` and `docs/handoff.md` as the current work queue. The frozen Phase 5A references are Binary with the accepted PV-01 signed-time-gap feature (254 features) and the conservative field-relative-drop LambdaRank (253 features). PV-00 through PV-05 are complete. Margin-aware rating improved as a standalone module but is not adopted as a LightGBM feature. Later completed PACE/SPEED evidence remains archived and reproducible, but is not an instruction to continue local feature search during Phase 5A. No new 2024 access is allowed; 2025 is forbidden.

Predictive EDA must enforce: 2013 quality/warm-up only, 2014–2019 discovery, 2020–2021 replication, 2022 confirmation, and target dates no later than 2022-12-31. Keep 2023 for its prior calibration role only. Treat one race as one choice set, use race/date-aware uncertainty, preserve strict same-date PIT emission-before-update, keep market data physically isolated, and never commit raw, cache, model, or recoverable runner-level artifacts.
