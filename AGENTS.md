...
The project is in **Phase 5B: EDA-guided representation / objective research**. Phase 5A and S1 Two-axis past-race value are complete. The human selected S3 Condition-adjusted performance target, which is now the only current experiment. S1 supported the historical performance residual, left standalone field quality inconclusive, and supported the joint arm without establishing two independently useful axes. Do not run S2 or any follow-up feature experiment automatically after S3.

Use `docs/experiments/s1_two_axis_race_value_20260901.md`, the S3 preregistration, machine-readable experiment summaries, and `docs/handoff.md` as the current decision state. Frozen controls remain Binary PV-01 signed-time-gap with 254 features and conservative field-relative-drop LambdaRank with 253 features; S1 did not automatically promote a production control. The LambdaRank PV-01 254-feature variant remains prospective-only. S3 must change the target formulation only, use the paired control feature scopes and 2020–2022 rolling folds, and keep S2 queued for post-S3 human review.

S1 preserved the Phase 5A firewall: 2013 warm-up and evaluation targets no later than 2022-12-31, with zero 2023/2024/2025 rows used. Future experiments must retain race choice sets, race/date-aware uncertainty, strict same-date PIT emission-before-update, and physical market isolation. Never commit raw, cache, model, or recoverable runner-level artifacts.
