...
Use `docs/model_research_priorities.md`, `docs/experiments/s_rank_model_research_conclusions_20260831.md`, and `docs/handoff.md` as the current work queue and decision state. PV-01's one signed time-gap feature remains the Binary development incumbent. The conservative 2024 LambdaRank reference remains the lean 253-feature model; lean plus the single SEC-3F history column is the accepted pre-2024 rolling candidate and has not opened a 2024 milestone. OPP-RECENT was not adopted, HPO-01 retained both parameter sets, fixed 50:50 ENS-01 was rejected, and removing new-horse races from Binary fitting was rejected. Retain the original top-three LambdaRank labels and new-horse races in training. The next modeling hypothesis is PACE-01; only a supported PACE-01 may lead to PACE-02. New hypotheses must first use the rolling-origin policy; 2024 is milestone-only and 2025 is not used for iterative selection. Do not tune PV-01, PV-06, the frozen rating, blend weights, or hyperparameters against 2024. LIVE-DATA archive groundwork is complete, but actual collection is blocked until a private supported Windows JV-Link host, contract/key, and transport exist. Do not jump to DNN, complex betting strategy, or UI without an explicit phase decision.
