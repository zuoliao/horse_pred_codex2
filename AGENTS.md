...
The project is in **Phase 5B: EDA-guided representation / objective research**. Phase 5A and S1 Two-axis past-race value are complete. S1 supported the historical performance residual, left standalone field quality inconclusive, and supported the joint arm without establishing two independently useful axes. Stop for human review. Do not run S2 race-wise probability or S3 performance target automatically.

Use `docs/experiments/s1_two_axis_race_value_20260901.md`, its preregistration, machine-readable experiment summary, and `docs/handoff.md` as the current decision state. Frozen controls remain Binary PV-01 signed-time-gap with 254 features and conservative field-relative-drop LambdaRank with 253 features; S1 did not automatically promote a production control. The LambdaRank PV-01 254-feature variant remains prospective-only. S3 is the recommended next hypothesis because performance was the stable S1 signal; S2 remains queued. Either requires explicit human selection.
