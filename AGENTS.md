...
**Phase 5A: systematic exploratory data analysis and problem reformulation is complete and awaiting human selection.** Keep PV-06 continuation, margin-token refinement, rating transformations, COND-01, and every other local production-feature experiment paused; they are not rejected. Do not automatically implement a production feature or candidate model. The only eligible next experiments are the three prioritized in `docs/eda/15_next_research_roadmap.md`, and one may begin only after human selection.

Use `docs/eda/14_eda_synthesis.md`, `docs/eda/15_next_research_roadmap.md`, `docs/eda/12_cross_review.md`, and `docs/handoff.md` as the current decision state. The frozen references are Binary with the accepted PV-01 signed-time-gap feature (254 features) and the conservative field-relative-drop LambdaRank (253 features). PV-00 through PV-05 are complete. Margin-aware rating improved as a standalone module but is not adopted as a LightGBM feature. Later completed PACE/SPEED evidence remains archived and reproducible, but is not an instruction to resume local feature search. No new 2024 access is allowed for hypothesis selection; 2025 is forbidden.

The completed predictive EDA enforced: 2013 quality/warm-up only, 2014–2019 discovery, 2020–2021 replication, 2022 confirmation, and target dates no later than 2022-12-31. Preserve that contract for any replay or selected follow-up. Keep 2023 for its prior calibration role only. Treat one race as one choice set, use race/date-aware uncertainty, preserve strict same-date PIT emission-before-update, keep market data physically isolated, and never commit raw, cache, model, or recoverable runner-level artifacts.
