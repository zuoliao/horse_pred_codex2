...
The project is in **Phase 5B: EDA-guided representation / objective research**. Phase 5A is complete, and the human user selected S1 Two-axis past-race value. Execute only S1 as a preregistered four-arm rolling comparison, then stop. Do not run S2 race-wise probability or S3 performance target automatically.

Use `docs/experiments/s1_two_axis_race_value_preregistration.md` once committed, together with the EDA synthesis/roadmap/cross-review and `docs/handoff.md`, as the current decision state. Frozen controls are Binary PV-01 signed-time-gap with 254 features and conservative field-relative-drop LambdaRank with 253 features. The LambdaRank PV-01 254-feature variant improved every point metric but is interval-inconclusive and remains prospective-only. No 2024/2025 outcome or market information may enter S1 definition, parameter selection, or acceptance.

S1 must preserve the Phase 5A firewall: 2013 warm-up only and evaluation targets no later than 2022-12-31. Keep 2023 in its prior calibration role and out of S1 evaluation. Treat one race as one choice set, use race/date-aware uncertainty, preserve strict same-date PIT emission-before-update, keep market data physically isolated, and never commit raw, cache, model, or recoverable runner-level artifacts.

The pre-EDA feature queue is no longer generically paused. PV-06 seconds mapping is `deferred_by_eda`; margin-rating arithmetic, old runner-relative recent-opponent work, and the old broad COND-01 are `superseded_by_eda`; old field-relative restoration and new-horse exclusion are `rejected`. Transition reliability, connection compression, and last-3F relative remain future A-priority candidates after S1–S3.
