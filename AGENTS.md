...
Use `docs/model_research_priorities.md`, the PACE-01 through PACE-04 experiment reports, and `docs/handoff.md` as the current work queue and decision state. PACE-01's one 90-day race-relative early-position feature passed the rolling acceptance path for both families. PACE-02 rival pressure and PACE-04 normalized position gain were inconclusive for both families; PACE-03 final position was Binary inconclusive and LambdaRank rejected. Do not adopt or tune those three derivatives. The accepted pre-2024 rolling candidates remain PV-01 plus PACE-01 for Binary and lean plus SEC-3F plus PACE-01 for LambdaRank, each with 255 features and unchanged parameters; no 2024 milestone has been opened. OPP-RECENT was not adopted, HPO-01 retained both parameter sets, fixed 50:50 ENS-01 was rejected, and removing new-horse races from Binary fitting was rejected. Retain the original top-three LambdaRank labels and new-horse races in training. The next modeling hypothesis is SPEED-01, a single transparent condition-adjusted historical speed residual; freeze its expectation model and PIT semantics on pre-2022 evidence before rolling evaluation. New hypotheses must first use the rolling-origin policy; 2024 is milestone-only and 2025 is not used for iterative selection. Do not tune PV-01, PACE-01, the frozen rating, blend weights, or hyperparameters against 2024. LIVE-DATA archive groundwork is complete, but the user has deferred actual collection because the official JV-Link transport is Windows-only and the current machine is a Mac. Do not pursue an unofficial Mac transport or JRA Web scraping. Do not jump to DNN, complex betting strategy, or UI without an explicit phase decision.
